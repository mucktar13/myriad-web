...
export default function Header({ isAnonymous, user, profile, loading, isGuest }: Props) {
  ...
  const handleUnFriendRequest = () => {
    if (friendStatus !== null) cancelFriendRequest(friendStatus);
    toggleRemoveAlert();
  };
  ...
  const handleApproveFriendRequest = () => {
    if (friendStatus !== null) toggleRequest(friendStatus, FriendStatus.APPROVED);
  };

  const handleRejectFriendRequest = () => {
    if (friendStatus !== null) toggleRequest(friendStatus, FriendStatus.REJECTED);
  };
